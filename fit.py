...
def fit(
    ...
    Args:
        ...
        const_params: list of constant parameter names.
    ...
    """
    var_vals, const_vals = marshal_df(opt_df, var_params, const_params, stratifiers)
    x0 = np.concatenate(var_vals)
    args = (const_vals, var_params, const_params, stratifiers, infer_signal_fn)

    # assert no overlap between var_params and const_params
    assert not set(var_params).intersection(
        set(const_params)
    ), "Overlap between var_params and const_params"

    resid_init = residuals(x0, *args)

    print(
        f"Fitting {np.concatenate(var_vals).size} parameters with {resid_init.size} points."
    )
    ...
    result_df = unmarshal_df(
        var_vals_opt, const_vals, var_params, const_params, stratifiers
    )
...
def multiplex(
    vals: np.ndarray, col_name: str, stratifier: str, target_df: pd.DataFrame
) -> None:
    """
    ...
    Args:
        vals: Array of values to assign. Its length should match the number of unique stratifier values
              (or be 1 if no stratifier is used).
        col_name: Name of the column in target_df to populate with values.
        stratifier: Name of the column in target_df used for stratification. If None, no stratification is used.
        target_df: Target DataFrame to modify. The function operates on this DataFrame in-place.
...
def demultiplex(
    ...
    Args:
        ...
        stratifier: Name of the column used for stratification. If None, no stratification is used.

    Returns:
        Array of unique values. The length of the array will be equal to the number of unique
        stratifier values (or 1 if no stratifier is used).

    Raises:
        AssertionError: If the stratification is not unique (i.e., multiple values found within a group).
    """
    if stratifier is None:
...
def marshal_df(
    ...
    """
    Separates variable and constant parameters from a DataFrame into a format suitable for optimization.

    This function extracts variable parameters as lists of arrays (one array per parameter) and
    constant parameters as a DataFrame. Variable parameters can be stratified, resulting in multiple
    ...
    Args:
        ...
        const_params: List of column names in df corresponding to constant parameters.
        stratifiers: Dictionary mapping variable parameter names to their stratification columns.
                     If a parameter is not stratified, it should not be included in this dictionary.
...
def unmarshal_df(
    ...
    This function is essentially the inverse of marshal_df. It takes separate variable parameter arrays
    and a constant parameter DataFrame, and combines them into a single DataFrame. It handles the
    correct assignment of stratified variable parameters.

    Args:
        var_vals: List of numpy arrays, each containing values for one variable parameter.
        const_vals: DataFrame containing all constant parameter values.
        var_params: List of variable parameter names, corresponding to the arrays in var_vals.
        const_params: List of constant parameter names (should match columns in const_vals).
        stratifiers: Dictionary mapping variable parameter names to their stratification columns.
                     If a parameter is not stratified, it should not be included in this dictionary.

    Returns:
        A DataFrame combining all variable and constant parameters. The resulting DataFrame
        will have the same number of rows as const_vals, with variable parameters correctly
        assigned based on their stratification.
    """
    df = const_vals.copy()
